...
def same_structure_as(original,other):
    ...
    #print(True not in [isinstance(element,list) for element in original])
    if type(original)==type(other):
        original_structure = [[str(type(element)),len(str(element))] for element in original]
        other_structure = [[str(type(element)),len(str(element))] for element in other]
